---
layout: tool-doc
title: "booktx Concepts"
permalink: /tools/booktx/concepts/
nav_tool: booktx
docs_project: "booktx"
docs_variant: "release"
docs_ref: "v0.4.3"
docs_commit: "8fd2041aa75035c38c24d877e9f9dc320c5ea628"
---

<!-- GENERATED by sphinxpress. Do not edit by hand. -->

{% raw %}
<style data-sphinxpress-style="api">
.sphinxpress-doc {
  --sp-api-border: #5aa9d6;
  --sp-api-surface: #eaf5fb;
  --sp-api-text: inherit;
  --sp-api-name: #111827;
  --sp-api-type: #1268a8;
  --sp-api-source: #12823b;
  --sp-api-code: #d33a2c;
  --sp-api-code-border: #e5e7eb;
  --sp-api-code-surface: #ffffff;
}

.sphinxpress-doc dl.py,
.sphinxpress-doc dl.function,
.sphinxpress-doc dl.method,
.sphinxpress-doc dl.class,
.sphinxpress-doc dl.attribute,
.sphinxpress-doc dl.data,
.sphinxpress-doc dl.exception {
  margin: 1.5rem 0;
}

.sphinxpress-doc dl.py > dt.sig,
.sphinxpress-doc dl > dt.sig-object {
  box-sizing: border-box;
  display: block;
  width: 100%;
  margin: 0;
  padding: 0.55rem 0.65rem;
  border-top: 3px solid var(--sp-api-border);
  background: var(--sp-api-surface);
  color: var(--sp-api-text);
  font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas,
    "Liberation Mono", "Courier New", monospace;
  font-size: 0.92rem;
  font-weight: 600;
  line-height: 1.35;
  overflow-wrap: anywhere;
  word-break: normal;
}

.sphinxpress-doc dt.sig .sig-prename,
.sphinxpress-doc dt.sig .descclassname {
  color: var(--sp-api-name);
  font-weight: 700;
}

.sphinxpress-doc dt.sig .sig-name,
.sphinxpress-doc dt.sig .descname {
  color: var(--sp-api-name);
  font-weight: 800;
}

.sphinxpress-doc dt.sig .sig-param,
.sphinxpress-doc dt.sig .sig-return,
.sphinxpress-doc dt.sig .n,
.sphinxpress-doc dt.sig .p,
.sphinxpress-doc dt.sig .o,
.sphinxpress-doc dt.sig .default_value {
  white-space: normal;
}

.sphinxpress-doc dt.sig .property,
.sphinxpress-doc dt.sig .kt,
.sphinxpress-doc dt.sig .pre,
.sphinxpress-doc dt.sig a.reference {
  color: var(--sp-api-type);
}

.sphinxpress-doc dt.sig a.reference.internal[href*="_modules"],
.sphinxpress-doc dt.sig a.viewcode-link {
  margin-left: 0.75rem;
  color: var(--sp-api-source);
  font-family: inherit;
  font-size: 0.8rem;
  font-weight: 700;
  white-space: nowrap;
}

.sphinxpress-doc dt.sig a.headerlink {
  display: none;
}

.sphinxpress-doc dl.py > dd,
.sphinxpress-doc dl > dd {
  margin: 0;
  padding: 0.55rem 0 0 1.6rem;
}

.sphinxpress-doc dl.py > dd > :first-child,
.sphinxpress-doc dl > dd > :first-child {
  margin-top: 0;
}

.sphinxpress-doc code.literal,
.sphinxpress-doc tt.literal {
  padding: 0.08em 0.32em;
  border: 1px solid var(--sp-api-code-border);
  border-radius: 0;
  background: var(--sp-api-code-surface);
  color: var(--sp-api-code);
  font-size: 0.88em;
}

.sphinxpress-doc .field-list {
  margin: 0.75rem 0;
}

.sphinxpress-doc .field-list th,
.sphinxpress-doc .field-list dt {
  font-weight: 700;
}

.sphinxpress-doc .field-list td,
.sphinxpress-doc .field-list dd {
  padding-left: 0.75rem;
}

@media (max-width: 48rem) {
  .sphinxpress-doc dl.py > dt.sig,
  .sphinxpress-doc dl > dt.sig-object {
    font-size: 0.84rem;
  }

  .sphinxpress-doc dl.py > dd,
  .sphinxpress-doc dl > dd {
    padding-left: 0.85rem;
  }
}

@media (prefers-color-scheme: dark) {
  .sphinxpress-doc {
    --sp-api-border: #62b5e5;
    --sp-api-surface: #152733;
    --sp-api-name: #f3f4f6;
    --sp-api-type: #7dc8f5;
    --sp-api-source: #70d58d;
    --sp-api-code: #ff8a80;
    --sp-api-code-border: #475569;
    --sp-api-code-surface: #111827;
  }
}

@media print {
  .sphinxpress-doc dl.py > dt.sig,
  .sphinxpress-doc dl > dt.sig-object {
    border-top-color: #777;
    background: #f2f2f2;
    color: #000;
  }

  .sphinxpress-doc dt.sig a.reference.internal[href*="_modules"],
  .sphinxpress-doc dt.sig a.viewcode-link {
    display: none;
  }
}
</style>

<div class="sphinxpress-doc">
<section id="concepts">
<h1>Concepts</h1>
<section id="source-project">
<h2>Source project</h2>
<p>The source project is shared by all profiles:</p>
<ul class="simple">
<li><p><code class="docutils literal notranslate"><span class="pre">source/</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">.booktx/source-config.toml</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">.booktx/source-manifest.json</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">.booktx/names.json</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">.booktx/chapter-map.json</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">.booktx/chunks/</span></code></p></li>
</ul>
<p>Re-extraction updates this shared source state for every profile.</p>
</section>
<section id="translation-profile">
<h2>Translation profile</h2>
<p>A profile is an isolated translation effort under
<code class="docutils literal notranslate"><span class="pre">translations/&lt;profile&gt;/</span></code>. It owns target language and locale, identity
configuration, context, the translation store, version ledger, tasks, ingest
files, reviews, reports, and rebuilt output.</p>
<p>Project-root commands require an explicit <code class="docutils literal notranslate"><span class="pre">--profile</span> <span class="pre">PROFILE</span></code> when they read or
write profile-local data. A command run from a profile root resolves the profile
from the validated <code class="docutils literal notranslate"><span class="pre">.booktx-profile.json</span></code> marker and uses <code class="docutils literal notranslate"><span class="pre">.</span></code> as its project
argument. There is no project-wide selector and no implicit single-profile
resolution.</p>
</section>
<section id="state-of-truth">
<h2>State of truth</h2>
<p><code class="docutils literal notranslate"><span class="pre">translations/&lt;profile&gt;/translation-store.json</span></code> is the current
<code class="docutils literal notranslate"><span class="pre">TranslationStoreV2</span></code> record store. It contains nested translation versions and
review candidates. <code class="docutils literal notranslate"><span class="pre">translation-version-ledger.json</span></code> records version history.
Generated <code class="docutils literal notranslate"><span class="pre">translated/</span></code>, editor indexes, reports, and output files are derived
artifacts and can be rebuilt.</p>
<p><code class="docutils literal notranslate"><span class="pre">translations/&lt;profile&gt;/context.json</span></code> is authoritative context state.
<code class="docutils literal notranslate"><span class="pre">context.md</span></code> is its rendered view. Effective context views used by tasks are
snapshotted under <code class="docutils literal notranslate"><span class="pre">context-history/views/&lt;sha&gt;/</span></code>.</p>
</section>
<section id="versions-and-reviews">
<h2>Versions and reviews</h2>
<p>Versions are scoped inside one profile. A model or baseline policy change can
create a new dotted version. A chapter note changes the next task’s effective
context but does not create a dotted version by itself. Review candidates use
the separate <code class="docutils literal notranslate"><span class="pre">R&lt;pass&gt;.&lt;run&gt;</span></code> namespace and are selected only when their
provenance chain is valid.</p>
</section>
<section id="context-and-terminology">
<h2>Context and terminology</h2>
<p>Context is profile-local. To reuse approved policy between books, use an
explicit context pack. To align sibling profiles in one book, use <code class="docutils literal notranslate"><span class="pre">context</span> <span class="pre">sync</span></code>; each target keeps its own context files.</p>
<p>Use <code class="docutils literal notranslate"><span class="pre">booktx</span> <span class="pre">glossary</span></code> for human terminology decisions. Use <code class="docutils literal notranslate"><span class="pre">booktx</span> <span class="pre">termbase</span></code>
only for the advanced reusable preference storage surface.</p>
</section>
</section>
</div>
{% endraw %}