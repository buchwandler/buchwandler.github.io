---
layout: tool-doc
title: "spokenform PolyNorm diagnostic benchmark"
permalink: /tools/spokenform/main/polynorm/
...
<section id="polynorm-diagnostic-benchmark">
...
not a Spokenform API convention.</p>
<p>The source is pinned to dataset commit
<code class="docutils literal notranslate"><span class="pre">f3c67e047bea6b7c40bc2466c0fdaad51d8ce67d</span></code> in
<code class="docutils literal notranslate"><span class="pre">apple/ml-speech-polynorm-bench</span></code>. The dataset is CC BY-NC-ND 4.0. The first
download requires an explicit license acknowledgement:</p>
<div class="highlight-bash notranslate"><div class="highlight"><pre><span></span>python<span class="w"> </span>-m<span class="w"> </span>benchmarks.polynorm<span class="w"> </span>--accept-license
</pre></div>
</div>
<p>After downloading, offline and focused runs are available:</p>
<div class="highlight-bash notranslate"><div class="highlight"><pre><span></span>python<span class="w"> </span>-m<span class="w"> </span>benchmarks.polynorm<span class="w"> </span>--offline
python<span class="w"> </span>-m<span class="w"> </span>benchmarks.polynorm<span class="w"> </span>--offline<span class="w"> </span>--locale<span class="w"> </span>en-US<span class="w"> </span>--category<span class="w"> </span>Date
...
<code class="docutils literal notranslate"><span class="pre">failures.jsonl</span></code> and <code class="docutils literal notranslate"><span class="pre">failures.md</span></code> only; summary metrics and evaluated-case
counts still cover the complete run. With no threshold, all current failure
entries are stored.</p>
<p>The overlap files are not assumed to have equal lengths; the evaluator derives
...
upstream category and ground-truth text are never rewritten.</p>
<p>Cached JSONL data and the upstream license live under
<code class="docutils literal notranslate"><span class="pre">.cache/polynorm-bench/&lt;commit&gt;/</span></code>. Reports live under
<code class="docutils literal notranslate"><span class="pre">benchmark-results/polynorm/&lt;run-id&gt;/</span></code>; only metrics and metadata are intended
...
and render mode. <code class="docutils literal notranslate"><span class="pre">gate_metrics</span></code> keeps safety, owned, extended-candidate,
...
<code class="docutils literal notranslate"><span class="pre">und</span></code> or <code class="docutils literal notranslate"><span class="pre">Cent</span></code>; locale punctuation is selected before separator heuristics;
URLs, e-mail addresses, versions, and caller-protected spans remain literal by
default.</p>
<section id="structured-normalization-ownership">
<h2>Structured normalization ownership</h2>
<p>Spokenform claims accepted structured expressions as generated output. Those
output spans carry ownership and provenance through later stages, so
<code class="docutils literal notranslate"><span class="pre">abbr2words</span></code> and the plain-number pass cannot reinterpret words or digits that
were generated by a structured recognizer. The public <code class="docutils literal notranslate"><span class="pre">PreparedText</span></code> result
exposes these ranges as <code class="docutils literal notranslate"><span class="pre">reserved_spans</span></code>, while source/output offset mapping
still points each generated phrase to its original source token.</p>
<p>Current structured coverage includes lexical hashtags and mentions with
...
numerals can be promoted with <code class="docutils literal notranslate"><span class="pre">normalize_literals=True</span></code>; caller-protected spans
remain unchanged. Math, music, and biology use conservative context/shape
grammars rather than corpus-specific branches.</p>
</section>
<section id="benchmark-profiles">
<h2>Benchmark profiles</h2>
<p>The default report uses the public conservative contract with
<code class="docutils literal notranslate"><span class="pre">normalize_literals=False</span></code>; URL/email and version rows remain protected and
